...
import com.server.utils.mailUtil.MailUtil;
...
public class ExtentTestNGIReporterListener implements IReporter {
    ...
    public void generateReport(List<XmlSuite>  xmlSuites, List<ISuite> suites, String outputDirectory) {
        ...
        for (ISuite suite : suites) {
            ...
            Map<String, ISuiteResult>  result1 = suite.getResults();
            ...
            for (ISuiteResult r : result.values()) {
                ...
                PassSize= context.getPassedTests().size();;
                FailSize = context.getFailedTests().size();
                SkipSize = context.getSkippedTests().size();
        ...
        extent.flush();
//        sendmail(PassSize+FailSize+SkipSize,PassSize,FailSize,SkipSize);

    }
    private void sendmail(int sum,int pass,int fail,int skip){
        String templateName = "template.ftl";
        String filePath = GlobalConfig.getEmailReportPath();//+GlobalConfig.getSlash()+"index.html";
        Map<String, String> reportResult = new HashMap<String, String>();
        reportResult.put("sum",Integer.toString(sum));
        reportResult.put("passed",Integer.toString(pass));
        reportResult.put("failed",Integer.toString(fail));
        reportResult.put("Skip",Integer.toString(skip));
        reportResult.put("date", DateUtil.date());
        try {
            MailUtil.sendMyMailAndFileByTemplate( filePath,reportResult,
                    templateName);
        } catch (IOException e) {
    ...
    private void init() {
        ...
        htmlReporter.config().setDocumentTitle("自动化测试报告");//ReporterLogger.getReportName());
        htmlReporter.config().setReportName("");//ReporterLogger.getReportName());
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                while (logIterator.hasNext()){
                    ...
                    if(details.contains(ReporterLogger.splitTimeAndMsg)){
                        String time = details.split(ReporterLogger.splitTimeAndMsg)[0];
                        log.setTimestamp(getTimeDate(time));
                        log.setDetails(details.substring(time.length()+ReporterLogger.splitTimeAndMsg.length()));
                    }else{
    ...
    private Date getTimeDate(String millis) {
        Date date = null;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
        try {
            date = sdf.parse(millis);
